...
def parse_episodes(input_file, tokenizer, K=1, n_queries=1, n_samples=100, markers=True, balancing="soft", seed=123, ensure_positive=False, cache="cache/", eval_single=False):
    ...
    if cache is not None:
        ...
        if os.path.exists(filepath):
            try:
                ...
                parsed_docs = [parse_document(doc, tokenizer, markers=markers) for doc in tqdm(input_file, desc="Parsing documents")]
    ...
    for i in tqdm(range(n_samples), desc="Constructing episodes"):
        if eval_single:
            for rel_type in episodes[i]["labeled_relations"]:
                ...
                # pselect labels
                episode["exemplars"] = [select_labels(parsed_docs[doc_id], [rel_type]) for doc_id in episode["support"]]
                episode["test_examples"] = [select_labels(parsed_docs[doc_id], [rel_type]) for doc_id in episode["query"]]
            ...
            # pselect labels
            episode["exemplars"] = [select_labels(parsed_docs[doc_id], episode["labeled_relations"]) for doc_id in episode["support"]]
            episode["test_examples"] = [select_labels(parsed_docs[doc_id], episode["labeled_relations"]) for doc_id in episode["query"]]
...
def select_labels(document, relation_types):
...
def parse_document(document, tokenizer, markers=True):
    # parse doc
    em = None
    if markers:
        em = ("*", "*")
